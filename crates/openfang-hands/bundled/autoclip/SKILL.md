---
name: autoclip-hand-skill
version: "1.0.0"
description: "AutoClip Hand — same ffmpeg/yt-dlp/STT expertise as Clip Hand; product evolution per bundled/clip/REQUIREMENTS.md"
runtime: prompt_only
---

# AutoClip Hand — Video Clipping Expert Knowledge

## Structured JSON (V1)

AutoClip **requires** these files in the job working directory (see `HAND.toml`):

| File | Phase | Purpose |
|------|-------|---------|
| `metadata.json` | After intake | `duration_sec`, resolution, `has_audio` |
| `transcript.json` | After transcribe | `words[]` with `t0`/`t1`/`text` in seconds |
| `segments.json` | After analyze | `segments[]` with `start_sec`/`end_sec`/`title`/`rationale` |

Phase 5 must read `segments.json` with `file_read` before running `ffmpeg` for each clip.
